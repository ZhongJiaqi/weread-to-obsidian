...
好书从来不是读一遍就完的——真正的收获在重读。读的时候遇到特别有感悟的地方，随手划线、写下当时的想法，这些顿悟才是一本书最值钱的部分。但书一多问题就来了：在微信读书里攒下 33 本书、6500+ 条划线和想法之后，想温习某个观点，只记得「在某本书里见过」——一本一千多页的书要从头翻起，根本想不起它落在哪一页。所以做了这个：单文件 Python CLI，直连微信读书随官方 weread-skills 技能包开放的 Agent API（key 与 skill 通用），一条命令把一本书的全部划线、想法按章节收成一份完整的读书笔记，落进 Obsidian——复习、温故、回溯都在自己的知识库里完成，可检索、可双链引用，随时一键跳回 App 原文。
